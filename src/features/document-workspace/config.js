...
const worksheetBuilderConfig = {
  ...
  description: 'Set the class, subject and pattern — Project Atlas generates the questions and answer key for you.',
  presetLabel: 'Practice pattern',
  ...
  note: 'Real generators cover arithmetic practice (addition, subtraction, multiplication, division, mixed). Class / subject / board / chapter / topic are your context for now — full syllabus question banks come later. Use in workspace to add a Word layout and generate DOCX.',
  ...
  fields: [
    { id: 'institution', label: 'School / institution name', type: 'text', default: '', placeholder: 'e.g. Sunrise Public School', full: true },
    ...
    { id: 'studentName', label: 'Student name (blank line if empty)', type: 'text', default: '' },
    { id: 'rollNo', label: 'Roll No (blank line if empty)', type: 'text', default: '' },
    { id: 'subject', label: 'Subject', type: 'text', default: 'Mathematics' },
    { id: 'date', label: 'Date', type: 'text', default: '', placeholder: 'e.g. 06/07/2026' },
    { id: 'syllabusSource', label: 'Curriculum / syllabus source', type: 'select', default: 'builtin', options: SYLLABUS_SOURCES.map((source) => ({ value: source.id, label: source.label })) },
    { id: 'board', label: 'Syllabus / board name', type: 'text', default: 'CBSE', placeholder: BOARD_EXAMPLES },
    { id: 'chapter', label: 'Chapter', type: 'text', default: '' },
    { id: 'topic', label: 'Topic', type: 'text', default: 'Addition' },
    { id: 'subtopic', label: 'Subtopic / learning objective', type: 'text', default: '', full: true },
    { id: 'syllabusNotes', label: 'Paste syllabus / chapter / topic notes', type: 'textarea', default: '', full: true, placeholder: 'Optional — teacher context only. The app stores and shows this; it does not auto-read the syllabus yet.', showIf: { syllabusSource: 'custom' } },
    { id: 'operation', label: 'Practice pattern', type: 'select', default: 'addition', options: ARITHMETIC_OPERATIONS.map((operation) => ({ value: operation.id, label: operation.label })) },
    ...
    { id: 'min', label: 'Minimum number', type: 'number', default: 1, min: 0, max: 100000 },
    { id: 'max', label: 'Maximum number', type: 'number', default: 10, min: 0, max: 100000 },
    ...
    { id: 'includeAnswerKey', label: 'Include answer key', type: 'toggle', default: true },
...
const questionPaperBuilderConfig = {
  ...
  defaultMode: 'build',
  title: 'Paper setup / blueprint',
  description: 'Define the syllabus scope and paper rules — Project Atlas builds the section structure, marks and difficulty spread.',
  presetLabel: 'Paper pattern',
  previewTitle: 'Paper preview / blueprint',
  generatedColumns: QUESTION_PAPER_GENERATED_COLUMNS,
  generateLabel: 'Generate paper preview',
  note: 'The structure (sections, marks, difficulty spread, totals) is real. Question text is a clearly-labelled placeholder — real subject content comes from an uploaded question bank or future class/subject banks. Use in workspace to add a Word layout and generate DOCX.',
  ...
  fields: [
    { id: 'institution', label: 'School / institution name', type: 'text', default: '', placeholder: 'e.g. Sunrise Public School', full: true },
    { id: 'title', label: 'Paper title', type: 'text', default: 'Unit Test 1', full: true },
    { id: 'grade', label: 'Class / grade', type: 'text', default: 'Class 8' },
    { id: 'subject', label: 'Subject', type: 'text', default: 'Science' },
    { id: 'date', label: 'Date', type: 'text', default: '', placeholder: 'e.g. 06/07/2026' },
    { id: 'syllabusSource', label: 'Curriculum / syllabus source', type: 'select', default: 'builtin', options: SYLLABUS_SOURCES.map((source) => ({ value: source.id, label: source.label })) },
    { id: 'board', label: 'Syllabus / board name', type: 'text', default: 'CBSE', placeholder: BOARD_EXAMPLES },
    { id: 'chapterRange', label: 'Chapter range', type: 'text', default: '' },
    { id: 'topicRange', label: 'Topic range', type: 'text', default: '' },
    { id: 'syllabusNotes', label: 'Paste prescribed syllabus / chapter list / topic scope', type: 'textarea', default: '', full: true, placeholder: 'Optional — guides your setup. The app stores and shows this; it does not auto-generate real content yet.', showIf: { syllabusSource: 'custom' } },
    ...
    { id: 'questionType', label: 'Question type', type: 'select', default: 'Short answer', options: PAPER_QUESTION_TYPES.map((type) => ({ value: type, label: type })) },
    { id: 'difficultyDistribution', label: 'Difficulty distribution', type: 'select', default: 'balanced', options: QUESTION_DIFFICULTY_DISTRIBUTIONS.map((item) => ({ value: item.id, label: item.label })) },
    ...
    { id: 'showMarks', label: 'Show marks beside questions', type: 'toggle', default: true },
    { id: 'includeAnswerKey', label: 'Include answer key', type: 'toggle', default: false },
