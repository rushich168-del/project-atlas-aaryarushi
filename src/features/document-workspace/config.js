...
const questionPaperBuilderConfig = {
  ...
  fields: [
    ...
    { id: 'questionSourceMode', label: 'Question source', type: 'select', default: DEFAULT_QUESTION_SOURCE_MODE, options: QUESTION_SOURCE_MODE_OPTIONS, full: true, helper: 'Starter Question Bank uses available built-in questions. Teacher Pasted Material uses questions you paste, one per line. Teacher Reference / Topic creates original practice placeholders based on your selected topic and blueprint for teacher review. PDF upload is planned later.' },
    { id: 'questionBankScopeId', label: 'Question bank content', type: 'select', default: PLACEHOLDER_ONLY_SCOPE_ID, options: QUESTION_BANK_SCOPE_OPTIONS, helper: 'Only the listed Class 6 Mathematics starter bank has real questions. Placeholder-only keeps the existing blueprint flow.', full: true, showIf: { questionSourceMode: 'starter-bank' } },
    { id: 'teacherPastedMaterial', label: 'Paste your questions (one per line)', type: 'textarea', default: '', full: true, placeholder: 'Paste one question per line. Numbering like 1. or Q1) is cleaned automatically. Your text is treated as teacher-provided content and is not stored.', helper: 'Questions are used in the order you paste them. Where there are not enough pasted questions, labelled placeholders are added. Review before classroom use.', showIf: { questionSourceMode: 'pasted-material' } },
    { id: 'referenceBook', label: 'Reference / book name (optional)', type: 'text', default: '', placeholder: 'e.g. Class 6 Mathematics reference', helper: 'Used only as a label. Original practice questions are based on your topic and blueprint — book questions are not copied or reproduced.', showIf: { questionSourceMode: 'reference-topic' } },
    { id: 'referenceChapter', label: 'Reference chapter / unit (optional)', type: 'text', default: '', placeholder: 'e.g. Integers', showIf: { questionSourceMode: 'reference-topic' } },
    { id: 'teacherInstructions', label: 'Notes / instructions for yourself (optional)', type: 'textarea', default: '', full: true, placeholder: 'Optional notes to guide your own review. Stored only in this form for your reference.', showIf: { questionSourceMode: 'reference-topic' } },
    { id: 'questionVariant', label: 'Question Set', type: 'select', default: 'set-a', options: QUESTION_VARIANT_OPTIONS, helper: 'Set A, B, and C use deterministic selection from the same available bank. No hidden randomness.', showIf: { questionSourceMode: 'starter-bank' } },
    { id: 'refreshVariant', label: 'Refresh Variant', type: 'select', default: 'refresh-1', options: QUESTION_REFRESH_VARIANT_OPTIONS, helper: 'Same settings and same refresh produce the same paper. Change refresh to rotate available questions.', showIf: { questionSourceMode: 'starter-bank' } },
